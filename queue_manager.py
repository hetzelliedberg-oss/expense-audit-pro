...
def process_single_image(batch_id: str, file_name: str, file_path: str):
    ...
    try:
        ...
        # Step 2: Automatic Extraction (Gemini if key set, or Local OCR immediately!)
        from database import get_setting
        from ai_extractor import extract_with_local_ocr, extract_with_gemini
        ...
        if api_key:
            try:
                ...
                logger.warning(f"Gemini API error, falling back to local OCR: {e}")

        if not extracted_result:
            extracted_result = extract_with_local_ocr(file_path)
